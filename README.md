...
本项目将优秀的路由器透明代理项目 [Open-Box](https://github.com/liandu2024/Open-Box) 深度移植至 Android 操作系统，移除了所有依赖 OpenWrt（如 `uci`、`procd`、`dnsmasq`）的专有组件，针对移动端特殊的网络协议栈（Android `netd`、FBE 存储加密、多网卡切换、运营商 IPv6 投毒、ColorOS 异常防火墙拦截、热点客户端穿透）进行了系统级的深度重构与加固。
...
### 6. 无感热点与有线网络共享透明代理 (随身翻墙路由器)
* **把手机变成随身透明翻墙网关**：完美支持 **Wi-Fi 热点** 与 **USB 网络共享 (RNDIS)**。
* **连入设备开箱即用**：电脑、iPad、Switch、电视盒子等任何设备只要连接手机热点或插上 USB 共享，**无需在电脑上配置任何代理软件或证书，连上即可秒开 Google、YouTube、GitHub、X 等全部外网**！
* **双向透明拦截与 DNS 劫持**：在 Linux 内核 `PREROUTING` 链上同步拦截热点客户端发往公网的全部 TCP/UDP/DNS 流量，同时保持局域网设备互联互通。
* **支持大屏局域网跨设备管理**：连入热点的电脑或平板可在浏览器直接输入 `http://<手机热点IP>:2026`，在电脑大屏幕上实时查看流量图表、选择节点或配置规则。
...
## <a id="tethering"></a><a id="网络共享与热点"></a>📶 Wi-Fi 热点与 USB 网络共享指南

手机开启透明代理后，只需正常开启系统的网络共享，连入的设备即可全自动走代理上网：

1. **Wi-Fi 热点共享**：
   * 打开手机 **设置 → 连接与共享 → 个人热点**；
   * 电脑、iPad 或其他手机连接该热点，无需在电脑端设置任何 HTTP/Socks 代理，直接打开浏览器即可畅享 Google、YouTube；
2. **USB 有线网络共享 (RNDIS)**：
   * 用数据线将手机与电脑连接，在手机上开启 **“USB 网络共享”**；
   * 电脑网卡自动获取 IP 后，全系统所有软件（浏览器、Git、终端等）即刻获得极低延迟的有线透明代理加持；
3. **在电脑大屏上访问控制面板**：
   * 连入热点的电脑打开浏览器，输入手机的热点网关 IP 即可进入管理后台（例如 `http://192.168.43.1:2026` 或 `http://10.102.199.163:2026`）；
   * 输入你的管理密码，即可在大屏幕上轻松管理节点、测速及监控全网流量。
