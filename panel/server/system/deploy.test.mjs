...
test('冲突时不改系统(检测到 OpenWrt 插件在跑则拒绝部署)', async () => {
  // 冲突检测走 service.mjs 的 `sh <脚本> status`,所以 mock 键要带 sh 前缀。
  // Android 上这些路径通常不存在(静默跳过),这里保留功能本身的覆盖。
...
test('成功路径:写配置 + 重启内核 + 验证(Android 无 uci/防火墙步骤)', async () => {
  const ctx = okCtx()
  const r = await deployConfig(ctx, paths, { config, profile })
  ...
  assert.ok(ctx.writes.some((w) => w.path === paths.configPath))
  const c = cmds(ctx)
  // 内核重启走模块自带的 service-core.sh;防火墙与 DNS 接管由 scripts/iptables.sh 负责,
  // 部署流程不发任何 uci 命令(见 deploy.mjs 的 /sbin/uci 守卫)
  assert.ok(c.includes('sh /opt/open-box/scripts/service-core.sh restart'))
  assert.ok(!c.some((x) => x.includes('uci')), 'Android 部署不得发出 uci 命令')
})

test('DNS 重写源在 Android 下不生成 dnsmasq 文件,但元数据仍如实记录计划', async () => {
  // 上游 OpenWrt 会把重写源编成 dnsmasq 的 rebind-domain-ok 配置。Android 无 dnsmasq,
  // 该文件不落盘;部署仍须成功,且不得发出任何 uci 命令。
  const ctx = okCtx()
  ...
  assert.equal(ctx.files[`${paths.dataDir}/dnsmasq-forward.conf`], undefined, 'Android 不生成 dnsmasq 配置文件')
  assert.ok(!cmds(ctx).some((c) => c.includes('uci')), '不得发出 uci 命令')
  // 配置本身照常落盘
  assert.ok(ctx.writes.some((w) => w.path === paths.configPath))
})
...
test('IPv6 关闭时:v6 拦截交由 iptables.sh 接管,部署流程不发 uci 命令(红线 7)', async () => {
  const ctx = okCtx()
  const r = await deployConfig(ctx, paths, { config, profile: { ...profile, ipv6: false } })
  assert.equal(r.ok, true, r.message)
  // Android 无 uci:IPv6 DNS 的 53 端口阻断由 scripts/iptables.sh 的 OPENBOX_V6 链负责,
  // 部署流程本身不得再依赖 uci(见 deploy.mjs 里 fs.existsSync('/sbin/uci') 守卫)。
  assert.ok(!cmds(ctx).some((c) => c.includes('uci')), 'Android 部署不得发出 uci 命令')
})
...
test('内核二进制缺失 → 重启前预检拦截,精确归因而不依赖 procd 吞掉的退出码', async () => {
  ...
  const c = cmds(ctx)
  assert.ok(!c.includes('sh /opt/open-box/scripts/service-core.sh restart'), '二进制缺失时不应尝试重启内核')
  assert.ok(c.includes('sh /opt/open-box/scripts/service-core.sh stop'), '预检失败也要回滚到直连')
})
...
test('重启失败 → 回滚恢复直连', async () => {
  ...
  assert.equal(r.stage, 'start')
  const c = cmds(ctx)
  assert.ok(c.includes('sh /opt/open-box/scripts/service-core.sh stop'))          // 回滚停服务
...
test('dnsmasq 接管备份在 Android 下不参与部署(去 uci 化)', async () => {
  // 上游 OpenWrt 会用 uci 改写 dnsmasq 上游并在切模式时还原。Android 无 uci、无 dnsmasq,
  // 该备份文件即使存在也不会被消费,部署照常成功且不发任何 uci 命令。
  const ctx = createMockContext({
    files: {
      [dnsTakeoverBackupPath(paths)]: "dhcp.cfg01411c.server='223.5.5.5'\n",
      [paths.singbox]: '#!/bin/sh\n',
    },
    execResults: { ['sh ' + paths.initd.core + ' status']: { code: 0, stdout: 'running' } },
  })
...
test('落盘之后阶段抛出异常 → 回滚到直连并返回 stage:error', async () => {
  ...
  assert.ok(cmds(ctx).includes('sh /opt/open-box/scripts/service-core.sh stop'))    // 回滚:停服务
})
...
test('rollbackToDirect:每一步各自尽力、不抛,但失败要如实汇总,不再一律 ok:true', async () => {
  ...
  assert.deepEqual(r.failures.map((f) => f.step), ['stop-core'])
  assert.ok(r.failures.every((f) => /boom|失败/.test(f.message)))
  // 只有一步:Android 无 uci,restore-dns / remove-firewall 被 /sbin/uci 守卫跳过
  const fine = await rollbackToDirect(createMockContext(), paths)
  assert.equal(fine.ok, true)
  assert.deepEqual(fine.actions, ['stop-core'])
  assert.deepEqual(fine.failures, [])
})

// ---------- Android 架构哨兵:防止 OpenWrt 专有链路被误加回来 ----------
test('架构哨兵:Android 部署全程不发 uci / init.d 命令,也不碰 tun 模块', async () => {
  // 本项目 100% 专供 Android Root 环境:去 uci 化、无 dnsmasq、无 /etc/init.d、
  // 用 TPROXY 不依赖 tun 模块。部署链路若被误改回 OpenWrt 写法,这条会立刻失败。
  const ctx = okCtx()
  const r = await deployConfig(ctx, paths, { config, profile })
  assert.equal(r.ok, true, r.message)
  const c = cmds(ctx)
  assert.ok(!c.some((x) => x.includes('uci')), '不得发出 uci 命令')
  assert.ok(!c.some((x) => x.includes('/etc/init.d/')), '不得调用 OpenWrt init.d 脚本')
  assert.ok(!c.some((x) => x.includes('modprobe')), 'Android 用 TPROXY,不做 tun 模块加载')
  assert.ok(!c.some((x) => x.includes('logread')), 'Android 从 data/singbox.log 读日志,不依赖 logread')
  assert.ok(!c.some((x) => x.includes('opkg')), '不得出现 opkg 包管理调用')
  // 内核生命周期一律经模块自带脚本
  assert.ok(c.some((x) => x === 'sh ' + paths.initd.core + ' restart'), '内核重启走 service-core.sh')
})

test('架构哨兵:tun 设备缺失不影响 Android 部署(不再要求 kmod-tun)', async () => {
  // 上游会在没有 /dev/net/tun 时 modprobe 并提示装 kmod-tun。Android 走 TPROXY,
  // 该检查被 /sbin/uci 守卫跳过,所以即使 TUN_DEVICE 不存在也应部署成功。
  const ctx = okCtx()
  delete ctx.files[TUN_DEVICE]
  const r = await deployConfig(ctx, paths, { config, profile })
  assert.equal(r.ok, true, r.message)
  assert.ok(!cmds(ctx).some((x) => x.includes('modprobe')), '不该尝试加载 tun 模块')
  assert.ok(!/kmod-tun/.test(r.message || ''), '不该提示安装 kmod-tun')
})

test('架构哨兵:auto_redirect 降级链路在 TPROXY 架构下不会被触发', async () => {
  // AUTO_REDIRECT_FATAL 匹配的是 tun 入站 nftables 崩溃。Android 配置不含 tun 入站,
  // 即便内核 FATAL 文本命中该正则,也不应走「关掉 auto_redirect 重生成」的降级重试。
  assert.ok(AUTO_REDIRECT_FATAL.test('FATAL[0002] start service: post-start inbound/tun[tun-in]: auto-redirect: setup nftables: netlink receive: no such file or directory'))
  const ctx = okCtx()
  ctx.files[`${paths.dataDir}/singbox.log`] = 'FATAL[0002] start service: post-start inbound/tun[tun-in]: auto-redirect: setup nftables: netlink receive: no such file or directory\n'
  let rebuildCalls = 0
  const r = await deployConfig(ctx, paths, {
    config, profile,
    rebuild: () => { rebuildCalls += 1; return config },
  })
  assert.equal(r.ok, true, r.message)
  assert.equal(rebuildCalls, 0, 'TPROXY 架构下不该触发 auto_redirect 降级重建')
  assert.equal(cmds(ctx).filter((x) => x === 'sh ' + paths.initd.core + ' restart').length, 1, '只重启一次,不重试')
})

// ---------- Android 架构哨兵结束 ----------

test('重启失败且回滚也没成 → 提示写明恢复直连未完成、哪一步、为什么;不再笼统说"已恢复直连"', async () => {
  // Android 的回滚只有 stop-core 一步:让它也失败,验证提示如实汇总
  const ctx = createMockContext({
    files: { [paths.singbox]: '#!/bin/sh\n' },
    execResults: {
      ['sh ' + paths.initd.core + ' restart']: { code: 1, stderr: 'start failed' },
      ['sh ' + paths.initd.core + ' stop']: { code: 1, stderr: 'stop failed' },
    },
  })
  const r = await deployConfig(ctx, paths, { config, profile })
  assert.equal(r.ok, false)
  assert.equal(r.stage, 'start')
  assert.match(r.message, /start failed,恢复直连未完成\(stop-core: stop failed\)/)
  assert.equal(r.rollback.ok, false)
  // 回滚全成功时照旧说"已恢复直连"
  const fine = await deployConfig(createMockContext({ files: { [paths.singbox]: '#!/bin/sh\n' }, execResults: { ['sh ' + paths.initd.core + ' restart']: { code: 1, stderr: 'start failed' } } }), paths, { config, profile })
  assert.match(fine.message, /start failed,已恢复直连$/)
  assert.equal(fine.rollback.ok, true)
})


test('rollbackToDirect 在 Android 下只停内核,不发任何 uci 命令(防火墙由 iptables.sh 管理)', async () => {
  const ctx = createMockContext({})
  const p2 = createPaths('/opt/open-box')
  await rollbackToDirect(ctx, p2)
  const joined = ctx.calls.map((c) => [c.cmd, ...(c.args || [])].join(' ')).join('\n')
  assert.ok(joined.includes(p2.initd.core), '要停内核')
  assert.ok(!joined.includes('uci'), 'Android 回滚不得依赖 uci')
  assert.ok(!joined.includes('firewall.openbox'), '防火墙规则不由 uci 管理')
})
...
test('isCancelled 在落盘前为真 → 直接退出、什么都不动;在接管之后为真 → 回滚到直连;内核起了之后为真 → 不报成功也不回滚', async () => {
  // 1) 落盘前
  const early = okCtx()
  ...
  assert.ok(!cmds(early).some((c) => c.includes('restart')))
  // 2) 接管完成、内核还没起:内核重启之前取消 → 回滚到直连。
  //    Android 无 firewall reload 步骤,用「写正式配置」这一步(落盘)翻转标记,
  //    这样取消正好命中 deploy.mjs 第 253 行那个「落盘之后、重启之前」的检查点。
  let flipped = false
  const mid = okCtx()
  const realWrite = mid.writeFile.bind(mid)
  mid.writeFile = async (path, content) => {
    const res = await realWrite(path, content)
    if (path === paths.configPath) flipped = true
    return res
  }
  ...
  assert.ok(cmds(mid).includes('sh ' + paths.initd.core + ' stop'))
  assert.ok(!cmds(mid).some((c) => c.endsWith(' restart')), '取消发生在重启之前,不该重启内核')
  // 3) 内核已起(restart 之后)才取消:不报成功、不回滚,留给随后的停止动作
  let restarted = false
  const late = okCtx({ ['sh ' + paths.initd.core + ' restart']: () => { restarted = true; return { code: 0 } } })
  const r3 = await deployConfig(late, paths, { config, profile, isCancelled: () => restarted })
  assert.equal(r3.stage, 'cancelled')
  assert.equal(r3.ok, false)
  assert.ok(!cmds(late).includes('sh ' + paths.initd.core + ' stop'))
})
...
const redirectCtx = (fatal = REDIRECT_FATAL) => {
  const ctx = createMockContext({
    files: { [paths.singbox]: '#!/bin/sh\n', [TUN_DEVICE]: '' },
    execResults: {
      'sh /opt/open-box/scripts/service-core.sh status': () => {
...
test('geosite 转发名单在 Android 下只记录计划,不生成 dnsmasq 文件、不发 uci(第三轮 阶段 2)', async () => {
  // 上游 OpenWrt 会把走代理站点集的 geosite 展开成 dnsmasq 转发名单并写 conf-dir / 改 uci。
  // Android 无 dnsmasq、无 uci:部署照常成功,元数据仍如实记录计划结论供面板展示。
  ...
  assert.equal(r.ok, true, r.message)
  const meta = JSON.parse(ctx.writes.filter((w) => w.path === configMetaPath(paths)).pop().content)
  // 计划阶段的结论照常记下(纯函数,与是否 Android 无关)
  assert.equal(meta.firstLayer.dnsForwardPlanned, 'domains')
  assert.equal(meta.firstLayer.dnsMode, 'dnsmasq')
  // Android:不写 dnsmasq 配置文件、不发任何 uci 命令
  assert.ok(!ctx.writes.some((w) => w.path === '/tmp/dnsmasq.cfg.d/open-box.conf'), 'Android 不生成 dnsmasq conf')
  assert.ok(!cmds(ctx).some((c) => c.includes('uci')), 'Android 不得发出 uci 命令')
  // 关键词展不开时同样不落 dnsmasq 文件
  ...
  assert.ok(!cmds(ctx2).some((c) => c.includes('uci')), 'Android 不得发出 uci 命令')
})
