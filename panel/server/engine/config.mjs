...
export const buildConfig = ({ nodes, profile, userGroups, systemDns, localSubnets = [], directHostCidrs = [], subscriptions = [], ruleLists = {}, cacheFilePath = `${defaultRoot}/data/cache.db`, selections = {}, tlsCert = { certPath: `${defaultRoot}/etc/certs/server.crt`, keyPath: `${defaultRoot}/etc/certs/server.key` } }) => {
  ...
  // Android 下强制采用 hijack（内核协议嗅探劫持）模式，杜绝 dnsmasq 缺失与未劫持污染
